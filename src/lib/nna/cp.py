...
def CPibm16(coupling_map, qk, gates, next_gate):
    # Create the mip solver with the SCIP backend.
    solver = pywraplp.Solver.CreateSolver('SCIP')
    ...
    for g in gates:
        for i in range(num_grid):
            # 定义相邻位置矩阵
            near = []
            for j in range(len(coupling_map)):
                if coupling_map[j][0] == i:
                    near.append(coupling_map[j][1])
                if coupling_map[j][1] == i:
                    near.append(coupling_map[j][0])
            g0 = [g[0]]
            g1 = [g[1]]
            # p是位置，j是逻辑比特
            solver.Add(solver.Sum([x[p, j] for p in near for j in g1]+[1-x[i, g[0]]]) >= 1)
...
if __name__ == '__main__':
    ...
    res = CPibm16(coupling_map, 5, gates, [])
    print(res)
